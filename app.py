...
from financial_analyzer import FinancialAnalyzer, ValuationAnalyzer
...
def main():
    ...
    if analyze_button and ticker_input:
        ...
        try:
            # Initialize analyzer and valuation analyzer
            analyzer = FinancialAnalyzer(ticker_input)
            valuation_analyzer = ValuationAnalyzer(ticker_input)
            ...
            with val_tab1:
                ...
                if 'fcf_elements_data' not in st.session_state:
                    fcf_elements = valuation_analyzer.get_fcf_elements()
                ...
                if st.button("🔄 Calculate FCF Valuation", type="primary"):
                    ...
                    valuation_analyzer.save_fcf_assumptions(ticker_input, fcf_assumptions)
                    
                    # Calculate valuation with selected elements
                    fcf_result = valuation_analyzer.calculate_fcf_valuation(ticker_input, fcf_assumptions, cash_flow)
                    ...
                    if fcf_result:
                        ...
                        current_price = info.get('currentPrice', info.get('previousClose', 0))
                        if valuation_analyzer.save_valuation_results(ticker_input, 'FCF', fcf_result, current_price):
            ...
            with val_tab2:
                ...
                if st.button("🔄 Calculate RI Valuation", type="primary"):
                    ...
                    # Calculate valuation
                    ri_result = valuation_analyzer.calculate_ri_valuation(ticker_input, ri_assumptions, balance_sheet)
                    ...
                    if ri_result:
                        ...
                        # Save valuation results to database
                        current_price = info.get('currentPrice', info.get('previousClose', 0))
                        if valuation_analyzer.save_valuation_results(ticker_input, 'RI', ri_result, current_price):
                            st.info("💾 Valuation results saved to database")
